...
import { gradeSectionsService, studentsService, type GradeSection, type Student } from '@/services/mock-data'
...
const generateStaticAttendance = (students: Student[], date: string): Map<string, AttendanceStatus> => {
  ...
  students.forEach((student, index) => {
    // Generar datos variados para demostración
    let status: AttendanceStatus
    if (index % 10 === 0) {
      status = 'absent' // 10% ausentes
    } else if (index % 7 === 0) {
      status = 'late' // ~14% tardanzas
    } else if (index % 5 === 0) {
      status = 'pending' // algunos sin registrar
    } else {
      status = 'present' // mayoría presentes
    }
...
export default function AdminAsistenciaPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [gsData, studentsData] = await Promise.all([
  ...
  // Cargar datos estáticos cuando se selecciona un aula (para demostración)
  useEffect(() => {
  ...
  const levelGradeSections = useMemo(() => {
    if (!selectedLevel || selectedLevel === 'all') return gradeSections
                  ...
                  <div>
                    ...
                    <CardDescription>
                      {selectedGradeSectionData && (
                        <>
                          {selectedGradeSectionData.grade} {selectedGradeSectionData.section} · {selectedLevel}
      ...
      {selectedDate && !selectedGradeSection && (
        <Card className="border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              Selecciona un nivel y grado/sección para registrar la asistencia
            </p>
          </CardContent>
        </Card>
      )}
