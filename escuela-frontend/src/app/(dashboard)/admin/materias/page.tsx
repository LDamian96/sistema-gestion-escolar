...
// Asignación de nivel con sus grados-sección
...
interface FormData {
  ...
  gradeSection: string
  selectedGradeSections: string[] // Para selección múltiple dentro del nivel actual
  levelAssignments: LevelAssignment[] // Para múltiples niveles
}
...
export default function MateriasPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [subjectsData, gradeSectionsData] = await Promise.all([
  ...
  const filteredGradeSectionsForFilter = useMemo(() => {
    if (selectedLevel === 'all') return gradeSections
    return gradeSections.filter(gs => gs.level === selectedLevel)
  }, [gradeSections, selectedLevel])

  // Filtrar grado/secciones por nivel seleccionado en el formulario
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: 'Total Materias', value: subjects.length, icon: BookOpen, color: 'text-blue-500', bgColor: 'bg-blue-500/10' },
  ...
  // Actualizar formData cuando cambia el grado/sección
  ...
  // Toggle selección de grado-sección
  ...
  // Agregar nivel con sus grados-sección a la lista
  const addLevelAssignment = () => {
    if (formData.selectedGradeSections.length === 0) {
      showToast('error', 'Selecciona al menos un grado-sección antes de agregar')
      return
    }
    ...
    if (existingIndex >= 0) {
      // Agregar los nuevos grados-sección al nivel existente
      ...
      if (newIds.length === 0) {
        showToast('info', 'Los grados-sección seleccionados ya están agregados')
        return
      }
      ...
      showToast('success', `Se agregaron ${newIds.length} grado-sección(es) a ${formData.level}`)
    } else {
      // Agregar nuevo nivel
      setFormData(prev => ({
        ...prev,
      ...
      showToast('success', `Nivel ${formData.level} agregado con ${formData.selectedGradeSections.length} grado-sección(es)`)
    }
  }
  ...
  // Eliminar un grado-sección específico de un nivel
  const removeGradeSectionFromLevel = (level: string, gsId: string) => {
    setFormData(prev => ({
      ...
      levelAssignments: prev.levelAssignments
        ...
        .filter(la => la.gradeSectionIds.length > 0) // Eliminar niveles vacíos
    }))
  }

  // Obtener el total de grados-sección seleccionados (en la lista + selección actual)
  const getTotalSelectedCount = () => {
    const inList = formData.levelAssignments.reduce((acc, la) => acc + la.gradeSectionIds.length, 0)
    return inList
  }

  // Verificar si un grado-sección ya está en la lista de asignaciones
  ...
  const handleCreate = async () => {
    // Combinar selección actual con las asignaciones guardadas
    const allAssignments: LevelAssignment[] = [...formData.levelAssignments]

    // Si hay selección actual que no se ha agregado, incluirla
    if (formData.selectedGradeSections.length > 0) {
      const existingIndex = allAssignments.findIndex(la => la.level === formData.level)
      if (existingIndex >= 0) {
        // Agregar a nivel existente
    ...
    if (allAssignments.length === 0) {
      showToast('error', 'Debes agregar al menos un nivel con grados-sección')
      return
    }
    ...
    try {
      ...
      // Crear una materia por cada grado-sección de cada nivel
      for (const assignment of allAssignments) {
        for (const gsId of assignment.gradeSectionIds) {
          const gs = gradeSections.find(g => g.id === gsId)
          if (gs) {
            const dataWithColor = {
              name: formData.name,
              code: formData.code,
  ...
  const handleUpdate = async () => {
    ...
    if (!formData.gradeSectionId) {
      showToast('error', 'Debes seleccionar un grado/sección')
      return
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Administra las materias por grado y sección</p>
        </div>
            ...
            >
              <Card className="hover:shadow-lg transition-all hover:border-primary/50">
                <CardContent className="pt-6">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">Código: {subject.code}</p>
      ...
      {filteredSubjects.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            {searchTerm ? 'Intenta con otros términos de búsqueda' : 'Comienza creando una nueva materia'}
          </p>
        </div>
      )}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Código: {selectedSubject.code}</p>
                </div>
              </div>
      ...
      <Dialog open={modalMode === 'create' || modalMode === 'edit'} onOpenChange={closeModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Modo Crear: Lista de checkboxes para selección múltiple con soporte de varios niveles */}
            {modalMode === 'create' && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {filteredGradeSectionsForForm.length === 0 ? (
                    <div className="p-4 text-center text-muted-foreground border rounded-lg bg-muted/50">
                      No hay grado-secciones disponibles para el nivel {formData.level}
                    </div>
                  ) : (
                ...
                {(getTotalSelectedCount() > 0 || formData.selectedGradeSections.length > 0) && (
                  <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      <strong>Total:</strong> Se creará la materia para{' '}
