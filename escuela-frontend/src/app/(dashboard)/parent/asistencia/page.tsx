...
const generateMonthAttendance = (studentId: string, year: number, month: number): DayAttendance[] => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (!isWeekend && !isFuture) {
      const seed = (studentId + day.toString() + month.toString() + year.toString()).split('').reduce((a, b) => a + b.charCodeAt(0), 0)
      if (seed % 15 === 0) {
        status = 'absent'
      } else if (seed % 8 === 0) {
        status = 'late'
...
const generateYearAttendance = (studentId: string): DayAttendance[] => {
  const today = new Date()
  ...
  for (let month = startMonth; month <= today.getMonth(); month++) {
    const monthDays = generateMonthAttendance(studentId, currentYear, month)
...
export default function ParentAsistenciaPage() {
  ...
  const monthAttendance = useMemo(() => {
    ...
    return generateMonthAttendance(selectedChild.id, currentMonth.getFullYear(), currentMonth.getMonth())
  ...
  const fullYearHistory = useMemo(() => {
    if (!selectedChild) return []
    return generateYearAttendance(selectedChild.id)
  }, [selectedChild])
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-teal-600 via-cyan-600 to-blue-600 p-6 text-white">
        <div className="absolute inset-0 bg-grid-white/10 [mask-image:linear-gradient(0deg,transparent,rgba(255,255,255,0.5))]" />
        <div className="relative">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold">Asistencia de Mis Hijos</h1>
              <p className="text-white/80 mt-1">Registro mensual de asistencia</p>
            </div>
          ...
          <Card className="border-0 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
            <CardContent className="p-6">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {stats.rate >= 95 ? '¡Excelente! Ha alcanzado la meta' : `Faltan ${95 - stats.rate}% para alcanzar la meta`}
              </p>
            </CardContent>
          </Card>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold">{selectedChild.firstName}</h3>
                        <p className="text-xs text-muted-foreground">{selectedChild.gradeSection}</p>
          ...
          >
            ...
            >
              <div className="text-center">
                ...
                <p className="text-muted-foreground mb-2">{selectedDay.date}</p>
