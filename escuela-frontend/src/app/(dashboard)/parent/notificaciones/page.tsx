...
export default function ParentNotificacionesPage() {
  ...
  // Cargar notificaciones para PARENT
  useEffect(() => {
    loadNotifications()
  }, [])
  ...
  const loadNotifications = async () => {
    try {
      setLoading(true)
      // Obtener notificaciones dirigidas a padres o a todos
      const data = await notificationsService.getByRole('PARENT')
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold">{notifications.filter(n => n.message.toLowerCase().includes('pago')).length}</p>
              ...
              {filteredNotifications.map((notif, index) => {
                ...
                return (
                  <motion.div
                    ...
                    } ${notif.type === 'error' ? 'border-red-300' : ''}`}
                  >
